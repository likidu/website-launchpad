...
- { type: "featureMedia", props: { eyebrow?, title?, subtitle?, items: [{title, description (supports inline links via [label](url)), image: {image: {assetId?, url}, alt?, width?, height?}, video?: {src?, sources?: [{src, type?}], poster?, width?, height?, autoPlay?, loop?, muted?, controls?} (when present, renders a video instead of the image; use sources for multiple formats)}], startPosition?: "left"|"right", className? } }
